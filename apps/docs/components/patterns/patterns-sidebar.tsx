...
export function PatternsSidebar({ activePath }: PatternsSidebarProps) {
      ...
      <nav className="grid max-h-[28rem] gap-5 overflow-y-auto px-3 py-4 lg:max-h-[calc(100vh-var(--nav-height)-73px)]">
        <section className="grid gap-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary">{patternDocs.length}</Badge>
          </div>
          <div className="grid gap-0.5">
            {filteredPatterns.map((item) =>
              item.href ? (
                ...
                <div
                  className="flex items-center justify-between rounded-md px-2 py-1.5 text-muted-foreground/55 text-sm"
                  key={item.slug}
                >
                  <span>{item.name}</span>
