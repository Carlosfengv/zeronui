...
export function SidebarProvider({
  ...
  const isMobileForced = isMobileProp !== undefined;
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    ...
    [state, open, setOpen, isMobile, isMobileForced, openMobile, toggleSidebar],
  );
...
export function Sidebar({
  ...
  const { isMobile, isMobileForced, state, openMobile, setOpenMobile } =
    useSidebar();
  ...
  if (isMobile) {
    return openMobile ? (
      <div className="fixed inset-0 z-50 md:hidden" data-slot="sidebar-mobile">
        <button
        ...
        <div
          className={cn(
            "fixed inset-y-0 z-50 flex w-(--sidebar-width) max-w-[85vw] flex-col bg-sidebar text-sidebar-foreground shadow-xl",
  ...
  return (
    <div
      className={cn(
        ...
        isMobileForced ? "block" : "hidden md:block",
      )}
...
export function SidebarGroupAction({
  ...
  return asChild ? (
    <Slot {...actionProps}>{children}</Slot>
  ) : (
...
export function SidebarMenuSubButton({
  ...
  return asChild ? (
    <Slot {...subButtonProps}>{children}</Slot>
  ) : (
    <a {...subButtonProps}>{children}</a>
  );
}
